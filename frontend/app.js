...
async function loadComponents() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(data.detail || data.message || "Failed to load components");
        }
        ...
        data.components.forEach(comp => {
            ...
            if (comp.resourceType.includes("structure/page") || 
...
async function selectComponent(componentPath, clickedElement) {
    ...
    try {
        ...
        for (const [key, value] of Object.entries(data.fields)) {
            ...
            row.innerHTML = `
                <label>${key}</label>
                <input type="text" id="field-${key}" value="${value !== null ? value : ''}">
            `;
